...
my $___Fixed = "UnknownWordPenalty";
my $___RANGES = undef;
...
if ($___DECODER_FLAGS =~ /(^|\s)-(config|c) /
    || $___DECODER_FLAGS =~ /(^|\s)-(mode|m) /
    || $___DECODER_FLAGS =~ /(^|\s)-(input-file|f) /
    || $___DECODER_FLAGS =~ /(^|\s)-(output-file|o) /
    || $___DECODER_FLAGS =~ /(^|\s)-(nbest-path|nb) /
    || $___DECODER_FLAGS =~ /(^|\s)-(input-text|i) /
  ) {
  die "It is forbidden to supply any of -config, -mode, -input-file, -output-file, -input-text or -nbest-path in the --decoder-flags.\nPlease use only the --config option to give the config file that lists all the supplementary files.";
}
...
my $featList_file = "features.list";
...
if ($__PROMIX_TRAINING) {
  ...
  for (my $i = 0; $i < scalar(@__PROMIX_TABLES); ++$i) {
    ...
    my $filtered_config = "targoman_$i.ini";
...
=cut
$___CONFIG_ORIG = $___CONFIG;
$___CONFIG = "$___WORKING_DIR/targoman.conf";
$___CONFIG = ensure_full_path($___CONFIG);

if(! -e $___CONFIG){
        safesystem("cp $___CONFIG_ORIG $___CONFIG");
}
# we run decoder to check validity of config file and to obtain all the feature
# names
my $featlist = get_featlist_from_decoder($___CONFIG);
#$featlist = get_featlist_from_file($featList_file);
...
if (defined $___ACTIVATE_FEATURES) {
  ...
  for (my $i = 0; $i < scalar(@{$featlist->{"names"}}); $i++) {
    ...
    $featlist->{"enabled"}->[$i] = $enabled{$shName . "_" . $cnt{$name}};
    $cnt{$name}++;
  }
} else {
  # all enabled
...
while (1) {
  ...
  if ($__PROMIX_TRAINING) {
    ...
    $interpolated_config ="targoman.interpolated.ini"; 
  ...
  create_config($___CONFIG, "./run$run.targoman.conf", $featlist, $run, (defined $devbleu ? $devbleu : "--not-estimated--"), $sparse_weights_file, $need_to_normalize);
...
if($___RETURN_BEST_DEV) {
  ...
  print "copying weights from best iteration ($bestit, bleu=$bestbleu) to targoman.conf\n";
  ...
  create_config($___CONFIG_ORIG, "./targoman.conf", $best_featlist,
                $bestit, $bestbleu, $best_sparse_file, $need_to_normalize);
}
else {
  create_config($___CONFIG_ORIG, "./targoman.conf", $featlist, $run, $devbleu, $sparse_weights_file, $need_to_normalize);
}
...
#    print "decoder_config = $decoder_config\n";
    ...
    {
      my $nbest_list_cmd = "-nb $filename";
      $ENV{LD_LIBRARY_PATH} = "$___DECODER_LIB";
      $decoder_cmd = "$___DECODER $___DECODER_FLAGS  -c run$run.targoman.conf";
      $decoder_cmd .= " $decoder_config $nbest_list_cmd  -f $___DEV_F";
      
      $decoder_cmd .= " -o run$run.out 2> /dev/null";
    }
...
sub get_featlist_from_decoder {
  # run decoder with the given config file and return the list of features and
  ...
  if (-e $featlistfn && ! -z $featlistfn) {   # exists & not empty
    ...
    print STDERR "Asking Targoman for feature names and values from $configfn\n"; 
    $ENV{LD_LIBRARY_PATH} = "$___DECODER_LIB";
    my $cmd = "$___DECODER -c $configfn";
    $cmd .= " -m ShowWeights > $featlistfn 2> /dev/null";
    print STDERR "Executing: $cmd\n";
    safesystem($cmd) or die "Failed to run targoman with the config $configfn";
  }
...
sub create_config {
 ...
 if ($need_to_normalize) {
  ...
  # create new decoder config file by cloning and overriding the original one
...
#  delete($P{"config"}); # never output
  ...
  while(1) {
    ...
    if ($line =~ /^NBestPathSize\s*=\s*.*/){
	print $out "NBestPathSize=$___N_BEST_LIST_SIZE" ;
	$line = <$ini_fh>;
        next;
    }
    # skip until hit [parameter]
    if ($line !~ /^\[FeatureFunctions\]\s*$/) {
      print $out $line;
      $line = <$ini_fh>;
      next;
    }
