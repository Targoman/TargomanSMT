...
enum REO_MODEL_TYPE {REO_MSD, REO_MSLR, REO_MONO, REO_WCBoundary};
...
void addPhrase(SentenceAlignment &, int, int, int, int, string &, ofstream &);
bool isAligned (SentenceAlignment &, int, int);
void outPutSkipMeanVar(OpOnPhrase&, SentenceAlignment&);

bool allModelsOutputFlag = false;

bool wordModel = false;
bool phrnumModel = false;
...
bool boundaryWordsFlag = false;
OpOnPhrase* opOnObject;
...
int main(int argc, char* argv[])
{
  ...
  for(int i=6;i<argc;i++) {
    ...
    } else if (strcmp(argv[i],"--BoundaryWords") == 0) { // adding "BoundaryWords" switch
      boundaryWordsFlag = true;
    }
    else if (strcmp(argv[i],"--reorderingGraph") == 0) {
      char* modelname = argv[++i];
      if(strcmp(modelname, "phrNum") == 0) 
	  phrnumModel = true;
      else{
	  cerr << "extract: syntax error, unknown reorderingGraph model name: " << modelname << endl;
	  exit(1);
      }
	...
	if(strcmp(modelType, "msd") == 0)
	  wordType = REO_MSD;
	else if(strcmp(modelType, "WcBoundary") == 0)
	  wordType = REO_WCBoundary;
  ...
  string overflow = fileNameExtract + ".overflow";
  string phrnumPath = fileNameExtract + ".phrNum";
  ...
  if (phrnumModel){
    phrNumFile.open(phrnumPath.c_str());
    overFlow.open(overflow.c_str()); //error recognization
  }
...
void extract(SentenceAlignment &sentence) {
  ...
  opOnObject = new OpOnPhrase(sentence);
  
  
  PhraseInfo* fisrtPhrase=new PhraseInfo(-1,-1,-1,-1,0);
  opOnObject->add(fisrtPhrase);
  ...
  for(int startE=0;startE<countE;startE++) {
	...
	if (!out_of_bounds){
	  ...
	  for(int startF=minF;
		...
		addPhrase(sentence, startE, endE, startF, endF, orientationInfo, extractFileOrientation);
	      }
	      if(phrnumModel ){
		   PhraseInfo *phrase=new PhraseInfo(startE,endE,startF,endF,phraseID);
                   opOnObject->add(phrase);
                   phraseID++;
	      }
	    }
	}
  ...
  PhraseInfo *phrase=new PhraseInfo(countE, countE, countF, countF, phraseID);
  opOnObject->add(phrase);
  string s="";
     //opOnObject.graph->fillPathLen();
  if(phrnumModel){
     opOnObject->reorderingGraph();
     bool overflow = false;
     opOnObject->connectedToGraph(sentence, overflow);
     //opOnObject.calcCount(sentence);
     //cout << sentence.ID << " ," << opOnObject->setOfPhrases.size() << endl; 
     if(phrnumModel){
       if(overflow == false){
	outPutSkipMeanVar(*opOnObject, sentence);
      }else{
	overFlow << sentence.ID << endl;
      }
    }
  }
...
/******/
void outPutSkipMeanVar(OpOnPhrase& opOnObject, SentenceAlignment& sentence){
	string orientationB, orientationF, orientationInfo, orientationInfoPhr, countB, countF, phrNumsFirstToEnd, phrNumsEndToFirst;
	float count;
	bool flag = false;
	stringstream os, osPhr;
	for(int i = 0; i < opOnObject.setOfPhrases.size(); i++){
	  orientationB = "";
	  orientationF = "";
	  countB = "";
	  countF = "";
	  ...
	  osPhr.str("");
	  	...
	  	PhraseInfo phr = *opOnObject.setOfPhrases.at(i);
		int id = phr.ID;
		vector <PhraseInfo*> next = phr.nextPhrase;
		vector <PhraseInfo*> pre = phr.previosPhrase;
	     if(id != 0 && id != opOnObject.setOfPhrases.size()-1){
		map<int, long> phrNumberFirstToEnd = opOnObject.graph->getPathFirstToEnd(id);
		map<int, long>::iterator it;
		if(phr.endE != sentence.target.size()){
			for(int pr = 0; pr < pre.size(); pr++){
			   int delta =  phr.startF - pre.at(pr)->endF;
			   if(delta < 0)
			     if(abs(delta) > 1)
			       orientationB += "dleft:";
			     else
			   ...
			   os << pre.at(pr)->ID << "-" << id;
			   count = opOnObject.graph->getCount(os.str());
			   if(count > 1){
			     cout << "sorry! count is bigger of 1\n";
			     cout << count << " ,id: " << sentence.ID << " ,os.str(): " << os.str() << endl;
			     exit(-1);
			    }
			   os.str("");
			   os << count << ":" << delta ;
		...
		if(phr.endE != sentence.target.size()-1){
		  for(int nxt = 0; nxt < next.size(); nxt++){
		      int delta =  next.at(nxt)->startF - phr.endF;
		      if(delta < 0)
			 if(abs(delta) > 1)
			    orientationF += "dleft:";
			 else
		      ...
		      os << id << "-" << next.at(nxt)->ID;
		      count = opOnObject.graph->getCount(os.str());
		      os.str("");
		      os << count << ":" << delta << ":" << opOnObject.graph->getBeta(next.at(nxt)->ID);
		      orientationF += os.str() + " "; 
            	      os.str(""); 
		  }
		}
		
		for(it = phrNumberFirstToEnd.begin(); it != phrNumberFirstToEnd.end(); it++){
		  os << (*it).first << ":" << (*it).second << " ";
		}
		phrNumsFirstToEnd = os.str();
		os.str("");
		os << opOnObject.graph->getBeta(0) << " ||| ";
		orientationInfo = os.str() + orientationB + "||| " + orientationF + "||| " + phrNumsFirstToEnd;
		os.str("");
 		addPhrase(sentence, phr.startE, phr.endE, phr.startF, phr.endF, orientationInfo, phrNumFile);
	     }
	}
}
/*******/

void addPhrase( SentenceAlignment &sentence, int startE, int endE, int startF, int endF , string &orientationInfo, ofstream &out) {
// cout << orientationInfo << "\n";
  ...
  for(int fi=startF;fi<=endF;fi++) {
    ...
    if ((orientationFlag && !boundaryWordsFlag) || phrnumModel) out << sentence.source[fi] << " ";
  }
  if(boundaryWordsFlag){
    out << sentence.source[startF] << " ";
    if(endF - startF >= 1){
	out << sentence.source[endF] << " ";
    }
  }
  
  extractFile << "||| ";
  if (orientationFlag || phrnumModel) out << "||| ";

  // target
  for(int ei=startE;ei<=endE;ei++) {
    extractFile << sentence.target[ei] << " ";
    extractFileInv << sentence.target[ei] << " ";
    if ((orientationFlag && !boundaryWordsFlag) || phrnumModel) out << sentence.target[ei] << " ";
  }
  if(boundaryWordsFlag){
    out << sentence.target[startE] << " ";
    if(endE - startE >= 1){
	out << sentence.target[endE] << " ";
    }
  }
  ...
  if (orientationFlag || phrnumModel)
    out << orientationInfo;
