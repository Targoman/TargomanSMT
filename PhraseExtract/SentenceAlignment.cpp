...
void addBoundaryWords(vector<TokenInfo> &phrase)
{
  phrase.insert(phrase.begin(), TokenInfo("<s>"));
  phrase.push_back(TokenInfo("</s>"));
}
...
bool SentenceAlignment::processTargetSentence(const char * targetString, int, bool boundaryRules)
{
  ...
  //target = tokenize(targetString);
    target = input.parseRichIXML(targetString);
  if (boundaryRules)
...
//  source = tokenize(sourceString);
  source = input.parseRichIXML(sourceString);
  if (boundaryRules)
