...
private:
  ...
  void addPhrases(SentenceAlignment &, int, int, int, int, string &);
  void addPhrase(SentenceAlignment &, int, int, int, int, string &, int srcBitmask, int trgBitmask);
  void writePhrasesToFile();
  bool checkPlaceholders (const SentenceAlignment &sentence, int startE, int endE, int startF, int endF);
  pair<bool, int> checkIXMLTags(const SentenceAlignment &sentence, int startE, int endE, int startF, int endF, int bitmask);
  bool isPlaceholder(const string &word);
  bool isIXMLTag(const TokenInfo &word);
          ...
          for(int startF=minF;
            ...
            for(int endF=maxF;
              ...
              if(buildExtraStructure) { // phrase || hier
                ...
                addPhrases(sentence, startE, endE, startF, endF, orientationInfo);
              }
            }
        }
      }
...
void ExtractTask::addPhrases( SentenceAlignment &sentence, int startE, int endE, int startF, int endF , string &orientationInfo){
    assert(endF - startF < 30 && endE - startE < 30);
    int srcBitmask = 0, trgBitmask = 0;
    vector<int> tagIndexes;
    for(int i = startF; i <= endF; i++){
        if(isIXMLTag(sentence.source[i]))
            tagIndexes.push_back(i);
    }
    for(int bt = 0; bt < (1 << tagIndexes.size()); bt++){
        srcBitmask = 0;
        for(int i = 0; i < tagIndexes.size(); i++){
            if(bt & (1 << i)){
                srcBitmask |= (1 << (tagIndexes[i] - startF));
            }
        }
        pair<bool, int> checkResult = checkIXMLTags(sentence, startE, endE, startF, endF, srcBitmask);
        if(!checkResult.first)
            continue;
        trgBitmask = checkResult.second;
        addPhrase(sentence, startE, endE, startF, endF, orientationInfo, srcBitmask, trgBitmask);
    }
}

void ExtractTask::addPhrase( SentenceAlignment &sentence, int startE, int endE, int startF, int endF , string &orientationInfo, int srcBitmask, int trgBitmask )
{
  // source
...
//  }
  ...
  for(int fi=startF; fi<=endF; fi++) {
    if (m_options.isTranslationFlag()){
        if(srcBitmask & (1 << fi - startF))
            outextractstr << sentence.source[fi].getTag().toStdString() << " ";
        else
            outextractstr << sentence.source[fi].getValue().toStdString() << " ";
    }
    if (m_options.isOrientationFlag()){
        if(srcBitmask & (1 << fi - startF))
            outextractstrOrientation << sentence.source[fi].getTag().toStdString() << " ";
        else
        outextractstrOrientation << sentence.source[fi].getValue().toStdString() << " ";
    }
  }
  ...
  for(int ei=startE; ei<=endE; ei++) {
    if (m_options.isTranslationFlag()) {
        if(trgBitmask & (1 << (ei - startE)))
            outextractstr << sentence.target[ei].getTag().toStdString() << " ";
        else outextractstr << sentence.target[ei].getValue().toStdString() << " ";
    }
    if (m_options.isTranslationFlag()) {
        if(trgBitmask & (1 << (ei - startE)))
            outextractstrInv << sentence.target[ei].getTag().toStdString() << " ";
        else
            outextractstrInv << sentence.target[ei].getValue().toStdString() << " ";
    }
    if (m_options.isOrientationFlag()){
        if(trgBitmask & (1 << (ei - startE)))
            outextractstrOrientation << sentence.target[ei].getTag().toStdString() << " ";
        else
        outextractstrOrientation << sentence.target[ei].getValue().toStdString() << " ";
    }
  }
  if (m_options.isTranslationFlag()) outextractstr << "|||";
  ...
  if (m_options.isTranslationFlag()) {
    for(int fi=startF; fi<=endF; fi++){
        if(srcBitmask & (1 << (fi - startF)))
            outextractstrInv << sentence.source[fi].getTag().toStdString() << " ";
        else
            outextractstrInv << sentence.source[fi].getValue().toStdString() << " ";
    }
    outextractstrInv << "|||";
  }
...
//    for(int fi=startF; fi<=endF; fi++) {
//      outextractstrContext << sentence.source[fi].getValue().toStdString() << " ";
//    }
//    outextractstrContext << "||| ";

//    // target
//    for(int ei=startE; ei<=endE; ei++) {
//      outextractstrContext << sentence.target[ei].getValue().toStdString() << " ";
//      outextractstrContextInv << sentence.target[ei].getValue().toStdString() << " ";
//    }
//    outextractstrContext << "||| ";
//    outextractstrContextInv << "||| ";

//    for(int fi=startF; fi<=endF; fi++)
//      outextractstrContextInv << sentence.source[fi].getValue().toStdString() << " ";
...
//    else outextractstrContext << sentence.source[startF-1].getValue().toStdString();

//    outextractstrContextInv << " < ";
//    if (startE == 0) outextractstrContextInv << "<s>";
//    else outextractstrContextInv << sentence.target[startE-1].getValue().toStdString();

//    // write context to right
//    outextractstrContextRight << "> ";
//    if (endF+1 == sentence.source.size()) outextractstrContextRight << "<s>";
//    else outextractstrContextRight << sentence.source[endF+1].getValue().toStdString();

//    outextractstrContextRightInv << " > ";
//    if (endE+1 == sentence.target.size()) outextractstrContextRightInv << "<s>";
//    else outextractstrContextRightInv << sentence.target[endE+1].getValue().toStdString();
...
void ExtractTask::extractBase( SentenceAlignment &sentence )
{
  ...
  for(int startF=0; startF<countF; startF++) {
    for(int endF=startF;
        ...
        endF++) {
      for(int fi=startF; fi<=endF; fi++) {
        outextractFile << sentence.source[fi].getValue().toStdString() << " ";
      }
      outextractFile << "|||" << endl;
    }
  }
  ...
  for(int startE=0; startE<countE; startE++) {
    for(int endE=startE;
        ...
        endE++) {
      for(int ei=startE; ei<=endE; ei++) {
        outextractFileInv << sentence.target[ei].getValue().toStdString() << " ";
      }
...
pair<bool, int> ExtractTask::checkIXMLTags(const SentenceAlignment &sentence, int startE, int endE, int startF, int endF, int bitmask){
    int trgBitmask = 0;
//    std::cerr << "aligned Size" << sentence.alignedToS.size() << " " << startF << "-" << endF << std::endl;
    for (size_t pos = startF; pos <= endF; ++pos) {
      if(bitmask & (1 << (pos - startF))){

          if(sentence.alignedToS.at(pos).size() != 1)
              return pair<bool, int>(false, 0);
          else{
//              std::cerr << "getTargetPos" << std::endl;
              int targetPos = sentence.alignedToS.at(pos).at(0);
//              std::cerr << "targetPos: " << targetPos << std::endl ;
//                        std::cerr << sentence.target[targetPos].getValue().toStdString() << std::endl;
              if((sentence.alignedToT.at(targetPos).size() != 1) || !isIXMLTag(sentence.target[targetPos]))
                  return pair<bool, int>(false, 0);
              trgBitmask |= (1 << (targetPos - startE));
          }
      }
    }
    return pair<bool, int> (true, trgBitmask);
}

bool ExtractTask::checkPlaceholders (const SentenceAlignment &sentence, int startE, int endE, int startF, int endF)
{
  for (size_t pos = startF; pos <= endF; ++pos) {
    const string &sourceWord = sentence.source[pos].getValue().toStdString();
    if (isPlaceholder(sourceWord)) {
      if (sentence.alignedToS.at(pos).size() != 1) {
        ...
        int targetPos = sentence.alignedToS.at(pos).at(0);
        const string &otherWord = sentence.target[targetPos].getValue().toStdString();
  ...
  for (size_t pos = startE; pos <= endE; ++pos) {
    const string &targetWord = sentence.target[pos].getValue().toStdString();
    if (isPlaceholder(targetWord)) {
      if (sentence.alignedToT.at(pos).size() != 1) {
        ...
        const string &otherWord = sentence.source[sourcePos].getValue().toStdString();
        if (!isPlaceholder(otherWord)) {
          return false;
        }
      }
...
bool ExtractTask::isIXMLTag(const TokenInfo &word){
    if(word.getTag() == "")
        return false;
    return true;
}
