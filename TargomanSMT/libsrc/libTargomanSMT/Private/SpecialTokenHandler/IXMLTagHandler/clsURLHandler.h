...
{
public:
    explicit clsURLHandler(quint64 _instanceID): intfIXMLTagHandlerModule(this->tagName(), _instanceID)
    { }
    ~clsURLHandler();
    TargetRulesContainer_t getTargetRules(const QString& _token);
    QString getDefaultTranslation(const QString& _token);
private:
    TARGOMAN_DEFINE_TAG_HANDLER_MODULE(Targoman::NLPLibs::enuTextTags::Type::URL, URLHandler);
