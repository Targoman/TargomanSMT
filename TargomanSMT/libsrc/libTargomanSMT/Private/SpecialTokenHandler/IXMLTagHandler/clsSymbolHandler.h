...
{
public:
    explicit clsSymbolHandler(): intfIXMLTagHandlerModule(this->tagName())
    { }
    ~clsSymbolHandler();
    TargetRulesContainer_t getTargetRules(const QString& _token);
    QString getDefaultTranslation(const QString& _token);
private:
    TARGOMAN_DEFINE_TAG_HANDLER_MODULE(Targoman::NLPLibs::enuTextTags::Type::Symbol, clsSymbolHandler);
