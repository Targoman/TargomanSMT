...
{
public:
    explicit clsOrdinalsHandler(quint64 _instanceID): intfIXMLTagHandlerModule(this->tagName(), _instanceID)
    { }
    ~clsOrdinalsHandler();
    TargetRulesContainer_t getTargetRules(const QString& _token);
    QString getDefaultTranslation(const QString& _token);
private:
    TARGOMAN_DEFINE_TAG_HANDLER_MODULE(Targoman::NLPLibs::enuTextTags::Type::Ordinals, OrdinalsHandler);
