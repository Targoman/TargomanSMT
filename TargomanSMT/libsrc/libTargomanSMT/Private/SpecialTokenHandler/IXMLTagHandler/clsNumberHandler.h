...
{
public:
    explicit clsNumberHandler(): intfIXMLTagHandlerModule(this->tagName())
    { }
    ~clsNumberHandler();
    TargetRulesContainer_t getTargetRules(const QString& _token);
    QString getDefaultTranslation(const QString& _token);
private:
    TARGOMAN_DEFINE_TAG_HANDLER_MODULE(Targoman::NLPLibs::enuTextTags::Type::Number, clsNumberHandler);
